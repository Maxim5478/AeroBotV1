...
async function starts() {
	...
	client.on('qr', () => {
		client.sendMessage("6283146208804@s.whatsapp.net", `Bot Sudah Sudah Aktif`, MessageType.extendedText)
		console.log(color('[','white'), color('!','red'), color(']','white'), color('Scan QRnya Cepet'))
	})

	fs.existsSync('./Aatm.json') && client.loadAuthInfo('./Aatm.json')
	client.on('connecting', () => {
		start('2', 'Menyambungkan')
	})
	client.on('open', () => {
		success('2', 'Sukses, Sc by Aatm')
	})
	await client.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./Aatm.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
    ...
    try {
      ...
      if (anu.action == "add" && mem.includes(client.user.jid)) {
        client.sendMessage(anu.jid, "Haii", "conversation");
      }
  ...
  if (position !== null) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const apiKey = BarBarKey// contact me on whatsapp wa.me/6283146208804
...
let settingstatus = 0;
    if (new Date() * 1 - settingstatus > 1000) {
      ...
      await client.setStatus(`Aktif selama ${uptime} | spam/vc/telfon?block | Sewa Bot Only |Subscribe Aatm`).catch((_) => _);
      settingstatus = new Date() * 1;
			...
			function addMetadata(packname, author) {	
				if (!packname) packname = 'Herman'; if (!author) author = 'Bot';	
...
var imgs = await client.prepareMessage('0@c.us', thumb, image, { thumbnail: thumb })
            ...
            var ctlg = await client.prepareMessageFromContent(from, {
                "productMessage": {
                    ...
                    "businessOwnerJid": "6288228261539@s.whatsapp.net",
                }
...
client.sendMessage(from, buffer, video, {mimetype: 'video/mp4', quoted: mek, caption: 'Aatm'})
break
case 'tiktokwm':
if (!isUser) return reply(mess.daftar)
if (isBanned) return reply (mess.banned)
...
client.sendMessage(from, buffer, video, {mimetype: 'video/mp4', quoted: mek, caption: '*Aatm_*'})
break
case 'instafoto':
if (!isUser) return reply(mess.daftar)
if (isBanned) return reply (mess.banned)
...
break
  ...
  if (!isUser) return reply(mess.daftar)
  if (isBanned) return reply (mess.banned)
  if (args.length == 0) return reply(`Example: ${prefix + command} Aatm`)
  txt = args.join(" ")
  reply(mess.wait)
   ...
   break
		...
		case 'dellprem':  
                ...
                case 'hidetag':
                  ...
                  client.sendMessage(from, buffer, image, {quoted: mek, caption: '*Aatm*'})
                  break
                   case 'take':
